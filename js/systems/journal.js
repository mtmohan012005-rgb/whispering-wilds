...
class FieldJournal {
  ...
  addPhoto(photo) {
    ...
    // Diegetic save: snapshot filed into journal
    if (window.gameSaveManager) {
      window.gameSaveManager.saveGame('auto', 'photo_capture');
    }
  }

  unlockEntry(entryId) {
    this.unlockedEntries.add(entryId);
    // Diegetic save: new journal entry inked
    if (window.gameSaveManager) {
      window.gameSaveManager.saveGame('auto', 'journal_entry');
    }
  }
